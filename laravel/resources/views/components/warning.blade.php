<div id="errore_warning" class="fixed top-0 left-0 z-100 w-full"> 
    <div class="content-grid w-full">
        <div class="mx-auto max-w-3xl flex items-center gap-4 rounded-b-md bg-yellow-400 px-4 py-2 text-sm shadow-md">
            <svg viewBox="0 0 24 24" class="shrink-0 text-yellow-800 w-6 h-6 mr-2">
                <path fill="currentColor" d="M23.119,20,13.772,2.15h0a2,2,0,0,0-3.543,0L.881,20a2,2,0,0,0,1.772,2.928H21.347A2,2,0,0,0,23.119,20ZM11,8.423a1,1,0,0,1,2,0v6a1,1,0,1,1-2,0Zm1.05,11.51h-.028a1.528,1.528,0,0,1-1.522-1.47,1.476,1.476,0,0,1,1.448-1.53h.028A1.527,1.527,0,0,1,13.5,18.4,1.475,1.475,0,0,1,12.05,19.933Z"></path>
            </svg>

            <p class="flex-1">
                Error message goes here. It can be longer and will wrap onto
                multiple lines without moving the close button.
            </p>
            
            <button class="shrink-0 rounded-full px-2 text-lg font-semibold active:text-yellow-900">
                x
            </button>
        </div>
    </div>
</div>
